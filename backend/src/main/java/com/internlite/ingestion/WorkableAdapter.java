...
public class WorkableAdapter implements SourceAdapter {
    ...
    @Value("${app.ingestion.workable.accounts:workable}")
    ...
    public List<CanonicalJob> fetch() throws Exception {
        ...
        for (String raw : accountsCsv.split(",")) {
            ...
            try {
                // Workable public SPI: https://www.workable.com/spi/v3/accounts/{account}/jobs?state=published
                String url = String.format("https://www.workable.com/spi/v3/accounts/%s/jobs?state=published", account);
                ...
                for (JsonNode j : jobs) {
                    try { out.add(normalize(j, account)); } catch (Exception e) { log.warn("[workable:{}] skip: {}", account, e.getMessage()); }
                }
            } catch (Exception e) {
    ...
    private CanonicalJob normalize(JsonNode j, String account) throws Exception {
        ...
        if (location.isEmpty() && j.has("city")) location = j.get("city").asText("");
        String applyUrl = j.has("url") ? j.get("url").asText() : "https://apply.workable.com/" + account + "/j/" + id;
        ...
        return CanonicalJob.builder()
                ...
                .title(title.trim()).companyName(capitalize(account)).location(location)
